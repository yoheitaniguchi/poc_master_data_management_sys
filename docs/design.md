...
**決定**: CSVヘッダー行はテーブル定義JSONの`columnId`（`columnName`ではなく）と完全一致する
ものとする。`papaparse`の`header: true`オプションでパースし、得られたオブジェクトのキーを
そのまま`columnId`として扱う。日本語の`columnName`ではなく機械可読な`columnId`を採用する
理由は、テーブル定義JSON自体が`columnId`を一意識別子としているため（要求仕様書§5.1）、
CSV側も同じ識別子系列に揃えるのが最も単純で曖昧さがないため。

**Phase 4での追記**: SCR-2のCSVダウンロード（DO-7、全カラムそのまま出力）も同じ`columnId`を
ヘッダーとして出力する。当初は画面表示同様に`columnName`（日本語表示名）をヘッダーに使う案も
検討したが、それだとダウンロードしたCSVをそのまま取込画面（SCR-1）へ再投入できず（全カラムが
NotNullエラーになる）、DO-6/7とDO-5の間で往復可能なフォーマットになっていない問題があった
（`ux-reviewer`指摘）。`columnId`ヘッダーに統一することで、検索結果をCSVとして出力し一部修正して
再取込する、という自然な運用が成立する。画面上の一覧表示（`<th>`）は引き続き`columnName`を使う
（人が読む用途と、往復可能なファイル形式の用途を分離している）。

### 4.8 Web WorkerからのIndexedDBスキーマバージョン管理の扱い（Phase 3）

§4.3のバージョン管理（定義JSON群のハッシュを`localStorage`と比較し、差異があればDB削除・
再作成）は`localStorage`に依存するが、Web Workerから`localStorage`は参照できない。

**決定**: スキーマのバージョン管理（削除・再作成の判定）はアプリ起動時にメインスレッド側
（`initMasterDataAccess`）でのみ行う。CSV取込を実行するWeb Worker（`csvImport.worker.ts`）は、
起動時に確定済みのスキーマを前提として`idb`の`openDB(DB_NAME, 1)`で既存DBに接続するだけとし、
